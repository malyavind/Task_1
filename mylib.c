	...
	while(1) {
		...
		if(ret == 0) {
			...
			if (data->fds[0].revents & POLLIN) {  ///new client connection
				...
				if (Mysql_check_user(data->mysql, message.from)) {
				...
				res = Find_users_groups(data->mysql, message.from);
						...
						if (received > 0) {
							switch(message.todo) {
								case 1:	///User's request for incoming messages	
									pthread_mutex_lock(&mutex);				
									res = Mysql_find_msg(data->mysql, message.from);
									pthread_mutex_unlock(&mutex);
									if (mysql_num_rows(res) == 0) {
										...
										while( (row = mysql_fetch_row(res))) {																					
											...
											if (Is_msg_delivered(data->mysql, message.id, message.to) == FALSE)
											...
											if (message.new_msg == TRUE) {
												...
												if (Add_to_delivered(data->mysql, id, message.to) == 0) {
								...
								case 2:
									if (Is_user_registered(data->mysql, message.to) == TRUE) {
										///User sent message
										syslog(LOG_INFO,"%s: %s\t to %s\n", message.from, message.msg, message.to);
										
										if (Mysql_insert_msg(data->mysql, message.from, message.to, message.msg, message.delay) == 0) {
								...
								case 3:///User sent message to group
									pthread_mutex_lock(&mutex);
									res = Mysql_find_groups(data->mysql);
									pthread_mutex_unlock(&mutex);
									while( (row = mysql_fetch_row(res))) {
									...
									if(Is_group_exists(data->mysql, message.to) == TRUE) {
										pthread_mutex_lock(&mutex);
										res = Get_users_fromgroup(data->mysql, message.to);
										pthread_mutex_unlock(&mutex);
										if (mysql_num_rows(res) == 0) {
								...
								case 4:
									...
									pthread_mutex_lock(&mutex);
									res = Mysql_find_delivered(data->mysql, message.from);
									pthread_mutex_unlock(&mutex);
									if (mysql_num_rows(res) == 0) {
								...
								case 5:
									...
									while (recv(data->fds[i].fd, &message, sizeof(message), 0)) {
										if (Is_group_exists(data->mysql, message.msg) == FALSE) {
											if (Create_and_join(data->mysql, message.msg, message.from)) {
												message.todo = NEW;
									...
									if (Add_to_group(data->mysql, message.from, message.msg) == TRUE) {
										syslog(LOG_INFO,"Пользователь %s вступил в группу %s\n", message.from, message.msg);
										strncpy (message.msg, "Вы присоеденились к группе", sizeof(message.msg));
										send(data->fds[i].fd, &message.msg, sizeof(message.msg), 0);
									}											
								...
								case 6:
									...
									while (recv(data->fds[i].fd, &message, sizeof(message), 0)) {
										if (Is_user_ingroup(data->mysql, message.from, message.msg) == FALSE) {
											message.todo = FALSE;
											send(data->fds[i].fd, &message.todo, sizeof(message.todo), 0);
										}
										else {
									...
									if (Delete_user_fromgroup(data->mysql, message.from, message.msg) == 0) {
...
void init(void *args, char *ip, char *upt) {
	...
	data->mysql = NULL;
	data->mysql = mysql_init(data->mysql);
	if (!data->mysql) {
	...
	if(!mysql_real_connect(data->mysql, "212.17.20.186", "Dima", "Dm1Try1@", "mydb", 3307, NULL, 0)) {
